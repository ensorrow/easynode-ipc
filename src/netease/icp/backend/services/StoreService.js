...
(function() {
  ...
  const FILTER_CONDITION_ALL_B = 3;
  const FILTER_CONDITION_CHECKING_B = 1;
  const FILTER_CONDITION_PASSED_B = 2;
    ...
    putWebsiteb() {
      ...
      return function *() {
        ...
        try {
          ...
          r = yield conn.update(model);
          return r.affectedRows + r.insertId > 0;
        } catch (e) {
          EasyNode.DEBUG && logger.debug(` ${e},${e.stack}`);
          return false;
        } finally {
      ...
      return function *() {
        ...
        try {
          ...
          if (formData.baseinfo.hasOwnProperty('id')) {
            ...
            ret = yield conn.execQuery(sql, args);
            ...
            ret = yield conn.execUpdate(sql, args);
            id = ret.insertId;
          }
        } catch (e) {
          EasyNode.DEBUG && logger.debug(` ${e},${e.stack}`);
